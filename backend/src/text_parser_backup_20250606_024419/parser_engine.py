...
class OpenRouterLLM(ChatOpenAI):
    ...
    def __init__(self, model_name: str = None, **kwargs):
        ...
        if not model_name:
            logger.info("No model specified, using OpenRouter account default. Note: Ensure account default supports tool use for LangChain agents")
            ...
            # Warn if using a model that might not support tool use
            if 'qwen' in model_name.lower() and 'free' in model_name.lower():
                logger.warning(f"Model {model_name} may not support tool use functionality required by LangChain agents")
...
class MoveTool(BaseTool):
    ...
    name: str = "move"
    description: str = "Use this tool when the player wants to move or go somewhere. Input should be the direction or destination."
...
class LookTool(BaseTool):
    ...
    name: str = "look"
    description: str = "Use this tool when the player wants to look at, examine, or observe something. Input should be what they want to look at."
...
class TakeTool(BaseTool):
    ...
    name: str = "take"
    description: str = "Use this tool when the player wants to take, get, grab, or pick up an item from the environment. DO NOT use for 'take off' commands - use unequip tool instead. Input should be the item name."
...
class UseTool(BaseTool):
    ...
    name: str = "use"
    description: str = "Use this tool when the player wants to use, activate, or interact with an object. Input should be the object and any target."
...
class TalkTool(BaseTool):
    ...
    name: str = "talk"
    description: str = "Use this tool when the player wants to talk to, speak with, or communicate with someone. Input should be who they want to talk to and optionally what about."
...
class AttackTool(BaseTool):
    ...
    name: str = "attack"
    description: str = "Use this tool when the player wants to attack, fight, or engage in combat. Input should be the target and optionally the weapon."
...
class CastSpellTool(BaseTool):
    ...
    name: str = "cast_magic"
    description: str = "Use this tool when the player wants to cast a spell or use magic. Input should be the spell name and optionally the target."
...
class InventoryTool(BaseTool):
    ...
    name: str = "inventory"
    description: str = "Use this tool when the player wants to check their inventory, items, or equipment."
...
class DropTool(BaseTool):
    ...
    name: str = "drop"
    description: str = "Use this tool when the player wants to drop, discard, or put down an item. Input should be the item name."
...
class SearchTool(BaseTool):
    ...
    name: str = "search"
    description: str = "Use this tool when the player wants to search, look for hidden items, find secret compartments, or explore thoroughly. Input should be what they want to search for or where they want to search."
...
class UnlockTool(BaseTool):
    ...
    name: str = "unlock"
    description: str = "Use this tool when the player wants to unlock, open with a key, or pick a lock on a container or door. Input should describe what they want to unlock and optionally how (key or lockpick)."
...
class EquipTool(BaseTool):
    ...
    name: str = "equip"
    description: str = "Use this tool when the player wants to equip, wear, or wield an item. Input should be the item name and optionally the preferred slot."
...
class UnequipTool(BaseTool):
    ...
    name: str = "unequip"
    description: str = "Use this tool when the player wants to unequip, remove, take off, or doff an equipped item. This tool handles 'take off' commands for equipment. Input should be the item name or equipment slot."
...
class LangChainParserEnhancer:
    ...
    def _init_prompt_templates(self):
        ...
        self.context_template = PromptTemplate(
            ...
            template="""
            Analyze the following user input and identify its components:
            
            User Input: {user_input}
            spaCy Entities: {spacy_entities}
            
            Identify:
            - Primary action or verb
            - Target or object of the action
            - Any additional modifiers or context
            - The overall intent of the command
            
            Respond with a structured summary of the analysis.
            """
        )
...
class ParserEngine:
    ...
    def __init__(self):
        ...
        # Phase 4: Initialize RAG components for enhanced intent analysis
    ...
    def parse(self, input_text: str) -> Optional[ParsedCommand]:
        ...
        # Phase 4: RAG-enhanced intent analysis for complex queries
        rag_analysis = {}
        if self.rag_available and len(text_to_parse.split()) > 3:  # Only use RAG for longer, more complex inputs
            try:
                rag_analysis = self._extract_intents_with_rag(text_to_parse)
                if rag_analysis.get("rag_available") and rag_analysis.get("confidence", 0) > 0.5:
                    current_context["rag_intent_analysis"] = rag_analysis
                    self.logger.debug(f"RAG analysis: {rag_analysis['enhanced_intent']} (confidence: {rag_analysis.get('confidence', 0):.2f})")
                elif rag_analysis.get("rag_available"):
                    current_context["rag_intent_analysis_fallback"] = rag_analysis
                    self.logger.debug(f"RAG analysis (low confidence): {rag_analysis.get('enhanced_intent', 'unknown')}")
            except Exception as e:
                self.logger.warning(f"RAG intent analysis failed: {e}")
    ...
    def _init_rag_components(self):
        """Initialize RAG components for enhanced intent analysis."""
        try:
            self.logger.info("Initializing RAG components...")
            
            # Initialize SentenceTransformers embedding model
            self.embedding_model = SentenceTransformer('BAAI/bge-small-en-v1.5')
            self.logger.info("SentenceTransformers embedding model loaded")
            ...
            try:
                self.knowledge_collection = self.chroma_client.get_collection("text_realms_kb")
                doc_count = self.knowledge_collection.count()
                self.logger.info(f"Connected to ChromaDB knowledge base with {doc_count} documents")
                self.rag_available = True
            except Exception as e:
                self.logger.warning(f"Could not access ChromaDB collection 'text_realms_kb': {e}")
                self.rag_available = False
                
        except Exception as e:
            self.logger.warning(f"Failed to initialize RAG components: {e}")
            self.rag_available = False
            self.embedding_model = None
            self.chroma_client = None
            self.knowledge_collection = None

    def _extract_intents_with_rag(self, input_text: str, confidence_threshold: float = 0.7) -> Dict[str, Any]:
        """
        Use RAG to perform enhanced intent analysis on user input.
        
        Args:
            input_text: The user's input text
            confidence_threshold: Minimum confidence to trigger RAG analysis
        
        Returns:
            Dictionary containing RAG analysis results
        """
        if not self.rag_available or not self.embedding_model or not self.knowledge_collection:
        ...
        try:
            ...
            if not query_results['documents'] or not query_results['documents'][0]:
                return {
                    "rag_available": True,
                    "relevant_context": [],
                    "enhanced_intent": "No relevant context found",
                    "confidence": 0.1
                }
            ...
            metadatas = query_results['metadatas'][0] if query_results['metadatas'] else [{}] * len(relevant_docs)
            
            # Calculate relevance scores (convert distances to similarity scores)
            relevance_scores = [1 - dist for dist in distances]
            
            # Filter by relevance threshold
            relevant_context = []
            for i, (doc, score, metadata) in enumerate(zip(relevant_docs, relevance_scores, metadatas)):
                if score > 0.3:  # Only include sufficiently relevant context
            ...
            # Analyze intent based on retrieved context
            enhanced_intent = self._analyze_intent_from_context(input_text, relevant_context)
            
            # Calculate overall confidence
            max_relevance = max(relevance_scores) if relevance_scores else 0
            context_coverage = len(relevant_context) / 5  # Normalize by max results
            overall_confidence = (max_relevance + context_coverage) / 2
            ...
            self.logger.error(f"Error in RAG intent extraction: {e}")
            return {
                "rag_available": True,
                "error": str(e),
                "confidence": 0.1
    ...
    def _analyze_intent_from_context(self, input_text: str, relevant_context: List[Dict]) -> str:
        """
        Analyze the user's intent based on input text and retrieved context.
        
        Args:
            input_text: The user's input
            relevant_context: List of relevant context documents from ChromaDB
            
        Returns:
            Enhanced intent analysis string
        """
        if not relevant_context:
            return "No relevant context for intent analysis"
        
        # Simple intent analysis based on context types
        context_categories = []
        for ctx in relevant_context:
            text = ctx['text'].lower()
            
            # Categorize context types
            if any(keyword in text for keyword in ['craft', 'recipe', 'material', 'forge']):
                context_categories.append('crafting')
            elif any(keyword in text for keyword in ['magic', 'spell', 'enchant', 'rune']):
                context_categories.append('magic')
            elif any(keyword in text for keyword in ['lore', 'history', 'legend', 'tale']):
                context_categories.append('lore')
            elif any(keyword in text for keyword in ['creature', 'monster', 'beast', 'dragon']):
                context_categories.append('creatures')
            elif any(keyword in text for keyword in ['location', 'place', 'region', 'area']):
                context_categories.append('locations')
        
        # Analyze input intent based on context
        input_lower = input_text.lower()
        
        if any(keyword in input_lower for keyword in ['what', 'tell', 'explain', 'describe', 'about']):
            dominant_category = max(set(context_categories), key=context_categories.count) if context_categories else 'general'
            return f"Knowledge seeking: {dominant_category} information"
        elif any(keyword in input_lower for keyword in ['craft', 'make', 'create', 'forge', 'build']):
            return "Crafting intent with relevant materials/recipes found"
        elif any(keyword in input_lower for keyword in ['cast', 'magic', 'spell', 'enchant']):
            return "Magic-related intent with spell/enchantment context"
        elif any(keyword in input_lower for keyword in ['find', 'locate', 'where', 'go to']):
            return "Exploration/navigation intent with location context"
        else:
            return f"General intent with context in: {', '.join(set(context_categories)) if context_categories else 'multiple areas'}"
