...
class IdleNPCManager:
    ...
    def __init__(self, dialogue_generator=None, db_service=None):
        """
        ...
        # Fallback dialogue templates when no dialogue generator is available
        self.fallback_dialogue_templates = {
            'world_events_concern': [
                "looks concerned and mentions, \"Have you heard about the troubles lately?\"",
                "glances around nervously and whispers, \"Strange times we're living in, wouldn't you say?\"",
                "approaches you with a worried expression. \"What do you make of all this unrest?\""
            ],
            'friendly_check_in': [
                "smiles warmly. \"How are you finding your stay with us?\"",
                "gives you a friendly nod. \"Need anything? Don't hesitate to ask.\"",
                "waves casually. \"Haven't seen you around much. Everything going well?\""
            ],
            'local_knowledge': [
                "notices your interest in the surroundings. \"This place has quite a history, you know.\"",
                "shares, \"Interesting fact about this area - not many travelers know this...\"",
                "approaches thoughtfully. \"You seem like someone who appreciates local wisdom.\""
            ],
            'professional_inquiry': [
                "asks politely, \"Is there something I can help you with today?\"",
                "inquires, \"Looking for anything specific? I might be able to point you in the right direction.\"",
                "approaches with a businesslike demeanor. \"Might I interest you in my services?\""
            ],
            'curious_observation': [
                "watches you with interest. \"I don't see many visitors like yourself around here.\"",
                "tilts their head curiously. \"That's an interesting approach. Where did you learn that?\"",
                "observes you for a moment. \"You're not from around here, are you?\""
            ],
            'weather_comment': [
                "remarks, \"Unusual weather we're having, isn't it?\"",
                "looks to the sky. \"Weather's turning. Might want to find shelter soon.\"",
                "comments casually, \"Beautiful day for an adventure, wouldn't you say?\""
            ],
            'concern_for_player': [
                "gives you a concerned look. \"You seem troubled. Anything I can do?\"",
                "approaches carefully. \"Are you all right? You look like you've been through a lot.\"",
                "asks with genuine concern, \"Do you need any assistance? You look a bit lost.\""
            ]
        }
    ...
    def generate_npc_initiative(self, 
        ...
        try:
            ...
            # Try to generate dialogue using dialogue_generator if available
            if self.dialogue_generator:
                try:
                    dialogue_text = self.dialogue_generator.generate_dialogue(
                        ...
                        is_npc_initiated=True
                    )
                except Exception as e:
                    self.logger.warning(f"Could not use dialogue generator: {e}, falling back to templates")
            
            # Fallback to templates if dialogue_generator is unavailable or fails
            if not dialogue_text:
                templates = self.fallback_dialogue_templates.get(dialogue_theme, [])
                if templates:
                    dialogue_text = random.choice(templates)
                else:
                    dialogue_text = "approaches you and begins a conversation."
