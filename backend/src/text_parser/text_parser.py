"""
Text Parser Module (texting module)
...
class TextParser:
    ...
    def _init_action_patterns(self):
        ...
        self.dialogue_patterns = [
            ...
            r"ask\s+(.+)\s+about\s+(.+)", r"tell\s+(.+)\s+about\s+(.+)",
            r"greet\s+(.+)"
        ]
        ...
        self.inventory_patterns = [
            ...
            r"equip\s+(?:my\s+)?(.+)", r"unequip\s+(?:my\s+)?(.+)",
            r"drop\s+(?:my\s+)?(.+)"
        ]
        ...
        self.crafting_patterns = [
            r"craft\s+(.+)", r"make\s+(.+)", r"forge\s+(.+)", r"brew\s+(.+)",
            r"create\s+(.+)",
        ...
        self.examine_patterns = [
            r"(?:look|examine|inspect|check)\s+(?:at\s+)?(.+)", r"read\s+(.+)",
            r"search\s+(.+)", r"investigate\s+(.+)"
        ]
    ...
    def _parse_combat(self, command: str, result: Dict[str, Any]) -> None:
        ...
        for pattern in self.combat_patterns:
            match = re.search(pattern, command)
            if match:
                result["action"] = match.group(0).split()[
                    0]  # first word as action
                result["target"] = match.group(1).strip()
                return
    ...
    def _parse_crafting(self, command: str, result: Dict[str, Any]) -> None:
        ...
        if re.search(r"(?:check|view|see|list)\s+(?:crafting\s+)?recipes",
                     command):
